...
If the config tool can't connect, press **Reboot in update mode (backup)** instead and pick the OpenPPG controller from the list. It uses the controller's serial port, so it works even when the normal connection doesn't. See [Troubleshooting](#troubleshooting).
...
This means your computer can see the controller but won't let the config tool talk to it. The controller and cable are fine. There are two usual causes:

- **Another tab or program already has the controller open.** Only one page can talk to the controller at a time.
- **Windows didn't set up the controller's driver.** Windows is supposed to do this automatically when you first plug the controller in. If it fails once, it keeps failing on that computer. Windows 7 and 8 can't do it at all.

Try these in order:

1. Close every OpenPPG config tab and any other program that might use the controller (for example the Arduino IDE).
2. Unplug the controller, restart the computer, plug the controller back in, and open the config tool in just one tab.
3. Try a different computer. A Mac works best since it doesn't need a driver.
4. To update firmware without connecting, use **Reboot in update mode (backup)** under Upgrades, then drag the .uf2 file onto the USB drive that appears.

### No USB drive appears

The update drive only appears after the controller reboots into update mode. It won't show up just from plugging the controller in. Use **Reboot in update mode** (or the backup button) first.
